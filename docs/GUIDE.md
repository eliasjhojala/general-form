...
| `privileges_strict:` field reaches strong params for the wrong user | Since 0.0.141 `permit_fields` enforces `privileges_strict:` (fail-closed, like the view helpers); a field gated only by `privileges_strict:` is no longer permitted for users lacking that strict privilege. If a consumer relied on the old (leaky) behaviour, gate the field with `privileges:` as well. |
| `type: :files` attachments vanish after saving a form without touching the upload | Fixed in 0.0.141: the multiple file field renders with `include_hidden: false` (Rails ≥ 7) so an untouched field submits nothing instead of a blank array, which under Rails 8.1 replace-on-assign purged all existing attachments. Per-app hidden-companion workarounds become redundant (harmless) once on this version. |
